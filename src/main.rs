...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Outdated => {
            println!(
                "{} Zerobrew does not currently support checking for updates.\n",
                style("ℹ").cyan().bold()
            );
        ...
        Commands::Upgrade => {
            println!(
                "{} Zerobrew does not currently support bulk upgrades.\n",
                style("ℹ").cyan().bold()
            );
        ...
        Commands::Cleanup { force } => {
            ...
            if packages.is_empty() {
                println!(
                    "{} No migrated packages to clean up.",
                    style("ℹ").cyan().bold()
                );
            } else {
